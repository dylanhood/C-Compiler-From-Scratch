...
void vector_resize(struct vector *vector)
{
    ...
    Zero elements | may want to see if the index is already overflowed
...
{
    #if 0
    Save the state of this vector to its self
    #endif

    struct vector tmp_vec = *vector;

    #if 0
    Not allowed to modify the saves so set it to NULL
    when pushed to save the stack
    #endif
...
void vector_shift_right(struct vector *vector, int index, int amount)
{
    ...
    Do not need to shift anything if out of bounds
    So stretch the vector up to index+amount
...
void vector_peek_pop(struct vector *vector)
{
    ...
    Also invalidate any pointers pointing directly to the vector data
    #endif
...
void vector_pop(struct vector *vector)
{
    ...
    Popping from the vector may just decrement the index | freeing memory may not be needed
...
{
    #if 0
    No going back or an invalid element may be accessed out of bounds
    #endif
